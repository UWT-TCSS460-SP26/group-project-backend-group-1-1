# Meeting Minutes

## Sprint 1 — TMDB Proxy Planning

**Date:** April 14, 2026  
**Attendees:** Nate, Harleen, Harsimar, Jonathan

### Sprint Goal

Proxy TMDB data for movies and TV shows through our own API endpoints with transformed schemas, full OpenAPI documentation, and automated tests.

### API Design Decisions

- Movie endpoints: `GET /movies/popular`, `GET /movies/search`, `GET /movies/{id}`
- TV endpoints: `GET /tv/popular`, `GET /tv/search`, `GET /tv/{id}`
- All responses use our own schema — no raw TMDB JSON exposed
- TMDB API key stored as `API-KEY` environment variable, never committed

### Work Distribution

- Harsimar: movie search, TV search, TV popular routes
- Harleen: movie details, TV details routes
- Jonathan: test suite, OpenAPI documentation
- Nate: popular movies route, project setup, code quality tools

### Next Steps

- Each member implements assigned routes following the proxy pattern in `moviesPopular.ts`
- Open PRs for review before merging to main
- Confirm deployment on Render responds at public URL

---

## Sprint 0 — Team Formation

### Agenda Item 1

**Meeting Manager:** Nathaniel

### Agenda Item 2

**Meeting Scribe:** Nathaniel (temp)

### Agenda Item 3

#### What is your name/nickname and what do you prefer to be called?

#### Responses

- Hasimar: Sim
- Harleen: Harleen
- Jonathan: Jonathan
- Nathaniel: Nate

#### Where did you do Freshman/Sophomore year and/or where did you take 142/143? Did your 142/143 prepare you for this course?

#### Responses

- Hasimar: I did the 142/143 equivalent at Washington State University. I feel as though it did prepare me for this course.
- Harleen: 142/143 at Green River College.
- Jonathan: 142/143 at Green River Community College, retook 143 here at UWT for better grade.
- Nathaniel: 142/143 at South Seattle community college.

#### What are your programming strengths and weaknesses?

#### Responses

- Hasimar: Strengths: Java, Javascript, problem-solving. Weakness: API design and testing (still improving)
- Harleen: Strengths: object-oriented design, problem-solving, debugging. Weaknesses: code optimization, maintaining consistency across projects, staying efficient with newer tools/APIs
- Jonathan: Strengths: debugging, willing to learn new tools quickly, Weaknesses: can overthink implementation details, losing momentum as well
- Nathaniel: Strengths: system design, architecture, troubleshooting (sometimes). Weakness: API drift, losing momentum, versioning.

#### What other obligations take time away from your ability to work on this project?

#### Responses

- Hasimar: Other classes are also heavily project based and work commitments.
- Harleen: Other two classes are also project heavy classes.
- Jonathan: I work weekends 3pm-2:30am Saturday and noon-8pm on Sunday.
- Nathaniel: 20 credits this quarter.

#### What is something you want others to know about yourself?

#### Responses

- Hasimar: I like anime, video games, and recently picked up clay as a new hobby.
- Harleen: I love turning random ideas into actual projects, and outside of coding I'm really into fitness and trying new things.
- Jonathan: I am a CS mentor here at UWT. You will most often find me at the Dougan 110 computer lab Monday through Thursday.
- Nathaniel: I like Tea, I like watching CPPCON on youtube to learn about new things. I like watching the NBA – my favorite team is the 2014 Spurs.

### Agenda Item 4

#### Do you want to have a dedicated group leader?

#### Responses

- Hasimar: I prefer everyone sharing responsibility.
- Harleen: Yes, having a dedicated group leader could help keep the team organized, ensures clear communication, and keeps everyone accountable.
- Jonathan: I don't mind. I'm down for anything.
- Nathaniel: I like more distributed/democratized leadership (more ownership)

#### Who are the Subject Matter Experts (SME) for different areas? GUI, OO, Logic, Management, etc

#### Responses

- Hasimar: GUI, some OO, frontend logic, and some GIT
- Harleen: GUI, OO, Logic, Some Management
- Jonathan: OO, database, problem solving (leetcode type of questions), git
- Nathaniel: OO, some Git, some Management (YouTrack)

#### Students A and B pair program together while students C and D pair program together

#### Responses

- Hasimar: Open to pairing with anyone
- Harleen: I am open to pair with anyone (maybe whoever's schedules align more)
- Jonathan: I can pair with anyone
- Nathaniel: Open to pair with anyone, I know surface level web dev.

#### Student A is a dedicated tester/Unit test creator

#### Responses

- Hasimar: Open to it
- Harleen: N/A
- Jonathan: N/A
- Nathaniel: N/A

#### Consider your group's strengths and weaknesses. Pair a weak programmer with a strong programmer for pair programming sessions

#### Responses

- Hasimar: Comfortable with frontend and debugging, still working on backend skills. Open to pairing with anyone who wants to strengthen their skills together.
- Harleen: Strong in OOP and backend development; can pair with someone needing help with structure or understanding classes and design.
- Jonathan: More comfortable with backend stuff. But generally I'm open to pair with anyone regardless of skill-level (let's learn together)
- Nathaniel: Good with design/scope, and some networking. Open to pair with anyone unclear in those.

#### Who has Git experience and/or wants to dive into working with Git and GitHub to become the group's Git SME?

#### Responses

- Hasimar: I have some knowledge and experience with Git and GitHub and am comfortable helping out when needed.
- Harleen: Little experience with Git and GitHub (branching, pushing, resolving basic issues) and interested in becoming the group's Git SME and improving further.
- Jonathan: I have git experience. I can take over after Sprint 1 for git SME if no one else wants to.
- Nathaniel: Temp for SPRINT 0.

### Agenda Item 5

Discuss your concerns for the group project. Air any bad experiences from group work in the past. Discuss what you want to get out of this group project. Discuss strategies you think can work for a successful group project.

#### Responses

- Hasimar: No concerns, but I think having regular meetings will help us collaborate better. Focusing on starting tasks early would be good so we have time to handle issues.
- Harleen: I want to make sure everyone contributes equally and we avoid last-minute stress, since I've had past group experiences where work wasn't balanced. I'm hoping to improve collaboration skills and build a strong project by using clear roles, regular check-ins, and proper Git workflow.
- Jonathan: I've had no bad group-project experiences. I just want some experience in this domain (full-stack project). We should communicate early in the week for these sprints.
- Nathaniel: N/A, I'd like time for pair programming and meetings. Keeping in contact through sprints is important.

### Agenda Item 6

The group needs to meet synchronously (online is OK) AT LEAST 3 times a Week. What times/days work for everyone?

#### Responses

- Hasimar: Any day and time but preferably weekdays (I work weekends).
- Harleen: Any time Monday, Friday and weekends (can meet after class as well).
- Jonathan: Most days except weekends.
- Nathaniel: Any days, any time, except Tuesdays and Thursdays (can meet briefly after class ~30 min)

### Agenda Item 7

#### Wrap-up

#### Responses

- Hasimar: N/A
- Harleen: N/A
- Jonathan: N/A
- Nathaniel: N/A
